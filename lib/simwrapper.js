...
class SimWrapper {
	...
	async Init(rom, customRomData, customKeylog) {
		...
		this._model = rom;
		const modelPrefix = rom.substring(0, 2);                                  // get model type - CY (old ClassWiz) or EY (new ClassWiz)
		if (modelPrefix === 'EY') {
			this._modelType = MODEL_TYPE_CLASSWIZ_NEW;
			IM = IM_EY;
		} else if (modelPrefix === 'CY') {
			const verNum = rom.substring(2, 5) * 1;								// get version
			if (verNum >= 840) {								// check for ES PLUS-2
				this._modelType = MODEL_TYPE_ES_PLUS_2;
				IM = IM_ESP;
			} else {
				this._modelType = MODEL_TYPE_CLASSWIZ_OLD;
				IM = IM_CY;
			}
		} else if (modelPrefix === 'LY' || modelPrefix === 'GY') {
			this._modelType = MODEL_TYPE_ES_PLUS;
			IM = IM_ESP;
		} else if (modelPrefix === 'ES') {
			this._modelType = MODEL_TYPE_ES;
			IM = IM_ES;
		} else if (modelPrefix === 'FC') {
			this._modelType = MODEL_TYPE_FC;
			IM = IM_ES;
		} else if (modelPrefix === 'SL') {
			this._modelType = MODEL_TYPE_SOLAR;
			IM = IM_SOLAR;
		} else {
			console.error(`SimWrapper: Init: Unknown model type: ${modelPrefix}`);
			return;
		}
		this.IM = IM;
		...
		if (romStatus) {											// ROM .loaded
			...
			switch (this._modelType) {
				...
				case MODEL_TYPE_SOLAR:
					...
					this._SetDataMemorySize(0x008000, 0x08ffff);
